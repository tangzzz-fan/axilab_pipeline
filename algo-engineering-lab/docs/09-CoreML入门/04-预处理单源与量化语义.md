...
| 要么参数全在 App 侧显式应用，要么将来全烘进模型（T15） | 「训练一套、线上拍脑袋另一套」 |
...
负例：对 raw **双重** StandardScaler 再喂路径 1，概率须显著偏离正确路径。  
详见：[`../02-算法对齐口径/coreml-preprocess-inmodel.md`](../02-算法对齐口径/coreml-preprocess-inmodel.md)、[`../06-实验复盘/case-13-coreml-preprocess-inmodel.md`](../06-实验复盘/case-13-coreml-preprocess-inmodel.md)。
